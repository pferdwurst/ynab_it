...
module YnabIt
  class ETL
    ...
    def csv_filename(user, raw_filename)
      acct_id, start_date,end_date, ext = raw_filename.split(".")
      fldr = File.join(TxDownloader.FORMATTED_DIR, user, acct_id, start_date)
    ...
    def process_dir(user, raw_directory)
          ...
          unless ( !File.directory?(date_dir) )
            ...
            Dir.entries(date_dir).each do |dwnld|
              ...
              unless ( ( dwnld =~ /.txs$/).nil? )
                ...
                begin
                  output_file = csv_filename(user, dwnld)
